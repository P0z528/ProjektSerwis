    ...
    <div id="updateToast" class="toast align-items-center text-bg-primary border-0" role="alert" aria-live="assertive" aria-atomic="true" data-bs-autohide="false">
        <div class="d-flex">
            <div class="toast-body">
                <strong>Aktualizacja!</strong> Technik zgłosił nowe zapotrzebowanie.
...
<script>
document.addEventListener("DOMContentLoaded", function() {
    // Pobiera startową ilość zapotrzebowań przekazaną z kontrolera
    let currentCount = {{ $kpiZap ?? 0 }};

    setInterval(() => {
        fetch('/api/magazyn/check-updates')
            .then(response => response.json())
            .then(data => {
                if (data.count > currentCount) { // Gdy technik zamówi coś nowego
