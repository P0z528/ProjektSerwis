...
<div class="toast-container position-fixed bottom-0 end-0 p-3" style="z-index: 1055;">
    <div id="updateToast" class="toast align-items-center text-bg-primary border-0" role="alert" aria-live="assertive" aria-atomic="true" data-bs-autohide="false">
        <div class="d-flex">
            <div class="toast-body">
                <strong>Aktualizacja!</strong> W puli pojawiło się nowe zlecenie.
...
<script>
document.addEventListener("DOMContentLoaded", function() {
    // Pobiera startową ilość zleceń przekazaną z kontrolera
    let currentCount = {{ $kpiDoPodjecia ?? 0 }};

    setInterval(() => {
        fetch('/api/technik/check-updates')
            .then(response => response.json())
            .then(data => {
                if (data.count > currentCount) { // Jeśli jest więcej zleceń niż było
                    const toastEl = document.getElementById('updateToast');
                    const toast = new bootstrap.Toast(toastEl);
                    toast.show();
                    currentCount = data.count; // Aktualizujemy, żeby nie spamowało powiadomieniami
                } else if (data.count < currentCount) {
                    currentCount = data.count; // Aktualizacja w dół, gdy ktoś inny weźmie zlecenie (bez alertu)
                }
            })
            .catch(err => console.error(err));
    }, 10000); // 10000 ms = odpytuje serwer co 10 sekund
});
</script>
@endsection
